...
#include <string.h>
#include "game.h"
#include "parser.h" // Включаем заголовочный файл с объявлением функции parse_command
...
void play_game(struct game* game) {
    ...
    while (game->state == PLAYING) {
        ...
        struct command* cmd = parse_command(game->parser, input); // Вызываем функцию parse_command
        if (cmd != NULL) {
...
struct game* create_game() {
    ...
    // Initialize backpack
    new_game->backpack = create_backpack(10); // Передаем аргумент для создания рюкзака
